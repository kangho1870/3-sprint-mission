...
async function fetchUserProfile(id) {
    try {
        const response = await fetch(`${ENDPOINTS.BINARY_CONTENT}?ownerId=${id}`);
        if (!response.ok) throw new Error('Failed to fetch profile');
        const profile = await response.json();

        if (!profile || !profile.fileContentType || !profile.data) {
            throw new Error('No valid profile image returned');
        }

        // Convert base64 encoded bytes to data URL
        return `data:${profile.fileContentType};base64,${profile.data}`;
