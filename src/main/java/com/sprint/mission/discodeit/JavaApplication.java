...
//
//        logAll("메세지 삭제 후", messageService.getChannelMessages(channel.getId()), e ->
...
//        logAll("유저 참여 채널", channelService.findAllByUserId(user2.getId()), System.out::println);
    ...
    public static <T> void log(String action, Supplier<T> supplier) {
        System.out.println(action + " : " + supplier.get());
    }
    public static <T> void log(String action, BooleanSupplier supplier) {
        System.out.println(action + " : " + (supplier.getAsBoolean() ? (action + " 성공") :(action + " 실패")));
    }
    public static <T> void log(String action, BooleanSupplier supplier, String content) {
        System.out.println(action + " : " + (supplier.getAsBoolean() ? (action + " 성공 : " + content) :(action + " 실패 ")));
    }

    public static <T> void logAll(String action, Collection<T> list, Consumer<T> consumer) {
